...
ConVar g_BoomerKilled,g_ChargerKilled,g_SmokerKilled,g_HunterKilled,g_JockeyKilled,g_SpitterKilled,
	...
	g_hInfiniteAmmoTime, g_hStageComplete, g_hFinalMissionComplete, g_hWipeOutSurvivor, g_StartingMoney, g_MinimumMoney;
...
{
	{"pistol",			"Pistol", 			"100"},
	{"pistol_magnum",	"Magnum", 			"15000"},
	{"pumpshotgun",		"Pumpshotgun", 		"20000"},
...
static char item_alternate_name[][][] = 
{
	{"molotov",			"molly"},
	{"pipe_bomb", 		"pipe"},
	{"vomitjar", 		"bile"},
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_qbuy", QuickBuyCommand);
	RegConsoleCmd("sm_quickbuy", QuickBuyCommand);
	RegConsoleCmd("sm_qb", QuickBuyCommand);
	...
	g_StartingMoney = CreateConVar("sm_shop_starting_money", "5000", "New players start with this much money.", FCVAR_NOTIFY, true, 1.0);
	g_MinimumMoney = CreateConVar("sm_shop_minimum_money", "5000",	"Reset player's money on map load if player have less than this amount.", FCVAR_NOTIFY, true, 1.0);
	...
	g_StartingMoney.AddChangeHook(ConVarChanged_Cvars);
	g_MinimumMoney.AddChangeHook(ConVarChanged_Cvars);
...
public Action QuickBuyCommand(int client, int args)
{
	if(g_bEnable == false) {
		ReplyToCommand(client, "[TS] %T", "Not enough players", client, g_iPlayerRequired);
		return Plugin_Handled;
    ...
    {
        GetCmdArg(i, arg2, sizeof(arg2));
		Quick_Buy(client, arg2);
    }
...
public void OnClientCookiesCached(int client)
{
	if(g_bCookiesCachedEnable == true && !IsFakeClient(client))
	{
		...
		int iMoney = StringToInt(sMoney);
		if (iMoney == 0)
		{
			g_iCredits[client] = g_StartingMoney.IntValue;
		}
		else
		{
			g_iCredits[client] = iMoney < g_MinimumMoney.IntValue ? g_MinimumMoney.IntValue : iMoney;
		}
	}
}
...
public int Quick_Buy(int client, char[] ItemName)
{
	char resultString[32];
	int itemCost = 0;
	char displayName[32];
	if (FindItemUsingAlternateName(ItemName, resultString, sizeof(resultString)))
	{
		Format(ItemName, sizeof(resultString), resultString);
	}

	if (!GetInfoOfItem(ItemName, displayName, sizeof(displayName), itemCost))
	{		
		CPrintToChat(client, "[{olive}TS{default}] %s", "Invalid Item", client, ItemName);
		return;
	}

	if(CanClientBuy(client, ItemName) == true) 
	{
		if(g_iCredits[client] - itemCost < 0)
		{
			CPrintToChat(client, "[{olive}TS{default}] %T", "Not enough money", client, g_iCredits[client], itemCost);
		}
		else
		{
			if (strcmp(ItemName, "health_100") == 0)
			{
				GiveClientHealth(client, 100, displayName);
			}
			else if (strcmp(ItemName, "Teleport") == 0)
			{
				if (IsHandingFromLedge(client))
				{
					PrintToChat(client, "%T", "Can't buy when handing from ledge", client);
				}
				else
				{
					if( TeleportToNearestTeammate(client, displayName))
					{
						g_iCredits[client] -= itemCost;
					}
					else
					{
						PrintToChat(client, "%T", "No Any Other Survivors", client);
					}
				}
			}
			else if (strcmp(ItemName, "Kill Witches") == 0)
			{
				KillAllWitches(client, "Kill Witches");
			}
			else
			{
				GiveFunction(client, ItemName, displayName);
			}
			g_iCredits[client] -= itemCost;
			PrintToServer("Bought %d: %s", itemCost, displayName);
		}
	}
}

public bool FindItemUsingAlternateName(char[] params, char[] returnString, int length)
{
	for (int i = 0; i < sizeof(item_alternate_name); i++)
	{
		if (strcmp(params, item_alternate_name[i][1]) == 0)
		{
			Format(returnString, length, item_alternate_name[i][0]);
			return true;
		}
	}
	return false;
...
public bool GetInfoOfItem(char[] params, char[] displayName, int length, int &price)
{
	for (int i = 0; i < sizeof(weaponsMenu); i++)
	{
		if (strcmp(weaponsMenu[i][0], params) == 0)
		{
			Format(displayName, length, weaponsMenu[i][1]);
			price = StringToInt(weaponsMenu[i][2]);
			return true;
		}
	}

	for (int i = 0; i < sizeof(meleeMenu); i++)
	{
		if (strcmp(meleeMenu[i][0], params) == 0) 
		{
			Format(displayName, length, meleeMenu[i][1]);
			price = StringToInt(meleeMenu[i][2]);
			return true;
		}
	}

	for (int i = 0; i < sizeof(medicThrowableMenu); i++)
	{
		if (strcmp(medicThrowableMenu[i][0], params) == 0) 
		{
			Format(displayName, length, medicThrowableMenu[i][1]);
			price = StringToInt(medicThrowableMenu[i][2]);
			return true;
		}
	}

	for (int i = 0; i < sizeof(otherMenu); i++)
	{
		if (strcmp(otherMenu[i][0], params) == 0) 
		{
			Format(displayName, length, otherMenu[i][1]);
			price = StringToInt(otherMenu[i][2]);
			return true;
		}
	}

	for (int i = 0; i < sizeof(specialMenu); i++)
	{
		if (strcmp(specialMenu[i][0], params) == 0) 
		{
			Format(displayName, length, specialMenu[i][1]);
			price = StringToInt(specialMenu[i][2]);
			return true;
		}
	}
	return false;
...
public int ShopMenuHandler(Menu menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			{
				DisplayShopMenu(param1, view_as<EMenuType>(eWeaponMenu), weaponsMenu, sizeof(weaponsMenu));
			}
			if (strcmp(item, "meleeMenu") == 0)
			{
				DisplayShopMenu(param1, view_as<EMenuType>(eMeleeMenu), meleeMenu, sizeof(meleeMenu));
			}
			if (strcmp(item, "medicThrowableMenu") == 0)
			{
				DisplayShopMenu(param1, view_as<EMenuType>(eMedicThrowableMenu), medicThrowableMenu, sizeof(medicThrowableMenu));
			}
			if (strcmp(item, "othersMenu") == 0)
			{
				DisplayShopMenu(param1, view_as<EMenuType>(eotherMenu), otherMenu, sizeof(otherMenu));
			}
			if (strcmp(item, "specialMenu") == 0)
			{
				DisplayShopMenu(param1, view_as<EMenuType>(especialMenu), specialMenu, sizeof(specialMenu));
			}
		}
		...
		case MenuAction_Select:
		{
			...
			if(CanClientBuy(param1, specialMenu[index][0]) == true) 
			{
				...
				else
				{
					...
					{
						KillAllWitches(param1, "Kill Witches");
						g_iCredits[param1] -= itemMoney;
					}	
...
void DisplayShopMenu(int client, EMenuType eMenutype, const char [][][] array, const int size)
{
	...
	switch(eMenutype)
	{
		...
		case (view_as<EMenuType>(especialMenu)):
		{
