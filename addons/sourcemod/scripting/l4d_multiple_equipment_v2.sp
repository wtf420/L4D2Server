...
public Plugin myinfo =
{
	...
	author = "MasterMind420 & Ludastar & Pan Xiaohai & Marcus101RR & SauceMaster",
...
public void OnPluginStart()
{
	...
	l4d_me_cookie = RegClientCookie("l4d2_multiple_equipment_v2", "sm_me_mode for l4d2_multiple_equipment_v2.smx", CookieAccess_Protected);
	...
	RegConsoleCmd("sm_me_mode", sm_me_mode);

	AutoExecConfig(true, "l4d_multiple_equipment_v2");
...
public void OnClientCookiesCached(int client)
{
	...
	{
		char temp[4];
		GetClientCookie(client, l4d_me_cookie, temp, sizeof(temp));
		int iTempMode = StringToInt(temp);
		iMode[client] = iTempMode;
	}
}
...
public Action sm_me_mode(int client, int args)
{
	if (args < 1)
	{
		Menu menu = new Menu(ModeMenuHandler, MenuAction_Select | MenuAction_Cancel | MenuAction_End);
		menu.SetTitle("Select a mode for Multiple Equipment");
		menu.AddItem("mode0", "Single Tap");
		menu.AddItem("mode1", "Double Tap");
		menu.AddItem("mode2", "Shift + R");
	...
	int mode = StringToInt(temp);
	iMode[client] = mode;

	SetClientCookie(client, l4d_me_cookie, temp);
    //return Plugin_Continue;
}
...
public int ModeMenuHandler(Menu menu, MenuAction action, int param1, int param2)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			if (strcmp(item, "mode0") == 0)
			{
				iMode[param1] = 0;
			}
			if (strcmp(item, "mode1") == 0)
			{
				iMode[param1] = 1;
			}
			if (strcmp(item, "mode2") == 0)
			{
				iMode[param1] = 2;
			}
			
			char temp[4];
			IntToString(iMode[param1], temp, sizeof(temp));
			SetClientCookie(param1, l4d_me_cookie, temp);
		}
		case MenuAction_End:
			delete menu;
	}
