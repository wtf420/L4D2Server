...
#define CONFIG_FILENAME               "new_ammo_packs-l4d2"
...
{
	if (!FileExists("../addons/sourcemod/plugins/left4dhooks.smx"))
	{
...
void AmmoPackEffects(int client, int iIndex)
{
			...
			for (int i = 1; i <= MaxClients; i++)
			{
				if (IsClientInGame(i) && GetClientTeam(i) == 2 && IsPlayerAlive(i))
				{
					...
					if (!bFrozen[i])
					{
						...
						ScreenEffects(i, {0, 0, 255, 150}, RoundFloat((fFreezeDuration, 500.0)), 0);
					...
					else
					{
						...
						ScreenEffects(i, {0, 0, 255, 150}, RoundFloat((fTime[i][0], 500.0)), 0);
			...
			for (int i = 1; i <= MaxClients; i++)
			{
				if (IsClientInGame(i) && GetClientTeam(i) > 1 && !IsFakeClient(i))
				{
					...
					if (!bIsVisible || iBlocked != -1)
					{
						...
						{
							bFlashed[i] = true;
							ScreenEffects(i, _, RoundFloat((fFlashDuration, 500.0)) / 2, 3);
						...
						else
						{
							...
							ScreenEffects(i, _, RoundFloat((fTime[i][18], 500.0)), 3);
					...
					else
					{
						...
						{
							bFlashed[i] = true;
							ScreenEffects(i, _, RoundFloat((fFlashDuration, 500.0)), 3);
	...
	{
		PrintHintTextToAll("• %N Has Opened A •\n%s", client, sAPName[client]);
		CPrintToChatAll("{blue}[{default}Box{blue}] {blue}%N {default}Has Opened A {green}%s !", client, sAPName[client]);	
		sAPName[client][0] = '\0';
	}
}
